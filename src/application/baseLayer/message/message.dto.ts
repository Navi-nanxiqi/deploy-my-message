...
// 兼容 api.rest 的渠道类型定义（保留原 sendTypes），并新增 SendChannel 供服务与管道使用
...
export class sendMessageReqDto {
    ...
    @IsArray() @ValidateNested({ each: true }) @Type(() => sendWayDto) sendway: sendWayDto[];
}

// 单渠道数据请求
export class AnyChannelMessageDto {
    @IsString() @IsNotEmpty()
    title: string;
...
export class EmailMessageDto implements AnyChannelMessageDto {
    @IsString() @IsNotEmpty()
    title: string;
...
export class WeiXinMessageDto implements AnyChannelMessageDto {
    @IsString() @IsNotEmpty()
    title: string;
...
export class DingtalkMessageDto implements AnyChannelMessageDto {
    @IsString() @IsNotEmpty()
    title: string;
...
export class WebhookMessageDto implements AnyChannelMessageDto {
    @IsString() @IsNotEmpty()
    title: string;
...
// 自托管消息
export class MessageNestMessageDto implements AnyChannelMessageDto {
    @IsString() @IsNotEmpty()
    title: string;
...
export class CustomMessageDto implements AnyChannelMessageDto {
    @IsString() @IsNotEmpty()
    title: string;
